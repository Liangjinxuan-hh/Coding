...
图5用于说明“手势基础模式”下的控制流程，核心为“手势选环 + 方向动作执行 + 复位处理”。
...
该图体现基础手势控制中的双路径机制：常规路径负责选环与方向控制，异常路径负责复位优先级抢占，从而保障控制安全性与响应实时性。
...
如图5所示，手势基础控制流程采用“手势选环 + 方向动作”的主链路，并通过双手五指张开的复位手势建立高优先级中断机制。当复位手势触发时，系统立即下发停止指令并将所有圆环复位到初始位置。
